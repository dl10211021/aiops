...
def check_server_health(ip, username, password, port=22):
    ...
    # Corrected command string with robust escaping
    # Use simpler quoting to avoid Python string interpolation issues
    # { printf("%.1f/%.1f GB (%.0f%%)", $3/1024, $2/1024, $3/$2*100) }
    # We must escape the double quotes for the inner string, and ensure python doesn't interpret %f
    
    awk_mem = r"free -m | awk \'/Mem:/ { printf(\"%.1f/%.1f GB (%.0f%%)\", $3/1024, $2/1024, $3/$2*100) }\'"
