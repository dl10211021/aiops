...
def main():
    ...
    print(f"{'ID':<12} | {'Name':<40} | {'Type':<15} | {'CPU %':<8}")
    print("-" * 85)
    
    # Target OS types (expanding to include more potential server types)
            ...
            try:
                ...
                if not cpu and 'Attribute' in result:
                    for attr in result['Attribute']:
                        ...
                        if display_name in ['CPU利用率', 'CPU Utilization', 'CPU Usage']:
                            cpu = attr.get('Value')
                            break
                ...
                if cpu:
                    try:
                        ...
                        if cpu_val > 80:
                            ...
                            print(f"{m_id:<12} | {disp_name[:40]:<40} | {m_type:<15} | {cpu_val:>7.1f}%")
                            count += 1
                    except ValueError:
                        continue
            except Exception:
    ...
    print("-" * 85)
    print(f"Total resources with CPU > 80%: {count}")
