...
`static_react/` is generated by `npm run build` and is ignored by Git. Use `--allow-built-assets` only for an explicit exceptional release decision that intentionally stages built assets:
...
- `frontend_build_artifact`: `static_react` generated assets and TypeScript build cache. Keep out of Git under normal development and release flow.
...
5. Commit product changes in logical groups: backend, frontend, tests, docs, and deployment configuration.
...
- `static_react/assets` contains generated frontend files after local builds. The long-term state is generated locally or during image build, not stored in Git.
...
- `generated_frontend_assets`: built frontend assets to keep ignored and regenerate at deployment time.
