...
def _tool_call_text(tool_name: str) -> str:
    return f"{tool_label(tool_name)}（`{tool_name}`）"
...
def protocol_tool_guidance(protocol: str, asset_type: str, host: str) -> str:
    ...
    if protocol == "ssh":
        return (
            ...
            f"直接调用 {_tool_call_text('linux_execute_command')} 执行巡检命令；不要再编写连接脚本或尝试重新登录。"
        )
    if protocol == "winrm":
        if asset_type == "hyperv":
            return (
                f"连接状态：后端已经建立到 Hyper-V 主机 {host} 的 WinRM 会话。"
                f"直接调用 {_tool_call_text('winrm_execute_command')} 执行 Get-VM、Get-VMHost、Get-VMSwitch 等 PowerShell 巡检命令；"
                "不要把它当作 VMware/OpenStack/ZStack API，也不要重新登录。"
            )
        return (
            f"连接状态：后端已经建立到 Windows 目标 {host} 的 WinRM 会话。你已经在该系统上下文内，"
            f"直接调用 {_tool_call_text('winrm_execute_command')} 执行 PowerShell/CMD 巡检；不要再编写 WinRM/Python 连接脚本，"
    ...
    if protocol in SQL_PROTOCOLS:
        ...
        return (
            ...
            f"你当前连接的是数据库实例，不是操作系统 Shell；直接调用 {_tool_call_text('db_execute_query')} 执行 SQL 读取或经审批的变更，"
    ...
    if protocol == "redis":
        return f"当前 Redis 资产使用 {_tool_call_text('redis_execute_command')}，凭据由资产中心托管注入。"
    if protocol == "memcached":
        return f"当前 Memcached 资产使用 {_tool_call_text('memcached_execute_command')}，支持 version、stats、get、gets 等只读命令。"
    if protocol == "mongodb":
        return f"当前 MongoDB 资产使用 {_tool_call_text('mongodb_find')} 做只读查询，凭据由资产中心托管注入。"
    if protocol in DATABASE_HTTP_PROTOCOLS:
        return (
            f"当前 {asset_type or protocol} 是数据库管理接口资产，不是通用业务 API。"
            f"使用 {_tool_call_text('database_api_request')} 通过数据库自身查询/管理接口执行巡检或经审批的配置操作，"
            "凭据由资产中心托管注入；写入、删除索引、修改集群配置等操作必须走审批。"
        )
    if protocol in VIRTUALIZATION_API_PROTOCOLS:
        return (
            f"当前 {asset_type or protocol} 是虚拟化/私有云平台资产。"
            f"使用 {_tool_call_text('virtualization_api_request')} 通过平台 API 做只读巡检或经审批的配置操作，"
            "不要把它当作普通主机 Shell，也不要绕过资产中心凭据。"
        )
    if protocol in STORAGE_API_PROTOCOLS:
        return (
            f"当前 {asset_type or protocol} 是存储平台资产。"
            f"使用 {_tool_call_text('storage_api_request')} 执行对象存储、备份或存储管理面的只读巡检；"
    ...
    if protocol in SERVICE_PROBE_PROTOCOLS or asset_type in SERVICE_ASSET_TYPES:
        return (
            ...
            f"使用 {_tool_call_text('service_probe_request')} 做只读连通性、证书、端口或协议握手探测；"
        ...
        for tool_name in (
            ...
            if tool_name in active_names:
                return (
                    ...
                    f"使用 {_tool_call_text(tool_name)} 调用目标只读接口，Token、Basic Auth 等凭据由资产中心托管注入；"
                    "涉及配置变更、删除、重启、发布等操作必须走审批。"
                )
    if protocol in API_PROTOCOLS:
        return (
            f"当前 API/监控平台资产使用 {_tool_call_text('http_api_request')} 调用目标 API；"
            "Token、Basic Auth 等凭据由资产中心托管注入。"
        )
    if protocol == "snmp":
        return f"当前 SNMP 资产使用 {_tool_call_text('snmp_get')} 读取 OID，Community/SNMP 凭据由资产中心托管注入。"
    return (
        "当前真实资产没有专用原生协议工具时，应直接报告工具缺口；"
        f"{_tool_call_text('local_execute_script')} 只允许 VIRTUAL 技能研发会话使用，不能代替真实资产协议连接。"
    )
...
def protocol_tool_list(
    ...
    return "\n".join(_localize_prompt_tool_line(line) for line in lines)


def _localize_prompt_tool_line(line: str) -> str:
    if not line.startswith("- ") or ":" not in line:
        return line
    tool_name, description = line[2:].split(":", 1)
    localized = tool_label(tool_name)
    if localized == tool_name:
        return line
    return f"- {localized} (`{tool_name}`):{description}"
