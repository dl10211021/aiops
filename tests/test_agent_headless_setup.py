...
class AgentHeadlessSetupTests(unittest.TestCase):
    def test_prepares_headless_prompt_context_tools_and_permission_inheritance(self):
        ...
        self.assertEqual(run.context["prompt_modules"]["surface"], "headless")
        self.assertEqual(run.context["prompt_modules"]["mode"], "read_write")
        self.assertIn("delegated_task", run.context["prompt_modules"]["modules"])
        self.assertTrue(run.context["prompt_modules"]["enabled"]["skill_paths"])
