...
class TestToolCatalogRoutes(unittest.TestCase):
    ...
    def test_tool_center_keeps_built_in_tools_without_hermes_branding(self):
        ...
        self.assertEqual(tools["write_file"]["operation_mode"], "write")
        self.assertEqual(tools["write_file"]["approval_policy"], "always_required")
        self.assertEqual(tools["write_file"]["result_store_policy"], "audit_and_evidence")
        self.assertEqual(tools["write_file"]["metadata_version"], 2)
        self.assertIn("session_search", tools)
        self.assertEqual(tools["session_search"]["status"], "available")
        self.assertTrue(tools["session_search"]["model_exposed"])
