...
class AgentPromptTests(unittest.TestCase):
    ...
    def test_chat_prompt_keeps_credentials_tools_skills_and_memory_sections(
        ...
        self.assertIn("优先调用 `web_search`", prompt)
        self.assertIn("[联网资料研究与浏览器流程]", prompt)
        self.assertIn("搜索结果片段不能单独作为 AIOps 资料结论", prompt)
        ...
        self.assertIn("必须继续调用 `browser_navigate`", prompt)
        self.assertIn("不要停在“我再试试”这类半句话", prompt)
        self.assertIn("不要因为 `web_search` 没有直接数据就让用户自己打开链接", prompt)
