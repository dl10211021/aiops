...
import { getLLMConfig, updateLLMConfig, getAvailableModels, getEmbeddingConfig, updateEmbeddingConfig } from '@/api/client'
...
export default function LLMConfigModal() {
  ...
  useEffect(() => {
    ...
    getEmbeddingConfig().then((r) => {
      setEmbModel(r.data.model || '')
      setEmbDim(r.data.dim || 768)
    }).catch(() => {})
  }, [])
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (embModel) await updateEmbeddingConfig(embModel, embDim)
    ...
    <div className="fixed inset-0 bg-black/50 z-40 flex items-center justify-center" onClick={closeModal}>
      <div className="bg-ops-panel rounded-xl p-6 w-[500px] max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-4">
          ...
          <hr className="border-ops-surface0" />

          {/* Embedding config */}
          <div>
            <label className="text-xs text-ops-subtext">Embedding 模型</label>
            <input value={embModel} onChange={(e) => setEmbModel(e.target.value)}
              className="w-full bg-ops-dark border border-ops-surface1 rounded-lg px-3 py-2 text-sm text-ops-text mt-1 outline-none focus:border-ops-accent font-mono"
              placeholder="text-embedding-004" />
          </div>
          <div>
            <label className="text-xs text-ops-subtext">Embedding 维度</label>
            <input type="number" value={embDim} onChange={(e) => setEmbDim(parseInt(e.target.value) || 768)}
