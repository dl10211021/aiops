...
export function TrendStrip({ points }: { points: AlertTrendPoint[] }) {
  ...
  if (recent.length === 0) {
    return <div className="ops-data-panel py-12 text-center text-sm text-ops-subtext">暂无告警趋势数据</div>
  }
  return (
    <div className="ops-data-panel p-4">
      <div className="mb-3 flex items-center justify-between text-[11px] text-ops-overlay">
        <span>Y 轴：告警数量</span>
        <span>X 轴：日期</span>
      </div>
      <div className="flex h-48 items-end gap-2 border-l border-b border-ops-surface1/70 pl-3 pb-3">
        {recent.map((point) => {
          const total = Number(point.total || 0)
          return (
            <div key={point.date} className="flex h-full min-w-0 flex-1 flex-col justify-end gap-2">
              <div className="text-center font-mono text-[10px] text-ops-subtext">{total}</div>
              <div
                className="min-h-2 rounded-t-lg bg-gradient-to-t from-ops-accent/60 to-ops-success"
                style={{ height: `${Math.max(8, (total / max) * 100)}%` }}
                title={`${point.date}: ${total} 条告警`}
              />
...
export function InspectionTrendStrip({ points }: { points: InspectionTrendPoint[] }) {
  ...
  if (recent.length === 0) {
    return <div className="ops-data-panel py-10 text-center text-sm text-ops-subtext">暂无巡检趋势数据</div>
  }
  return (
    <div className="grid gap-2 md:grid-cols-2">
      {recent.map((point) => (
        <div key={point.date} className="ops-data-panel px-3 py-2">
          <div className="mb-2 flex items-center justify-between text-xs">
            <span className="font-mono text-ops-text">{point.date}</span>
            <span className="text-ops-success">成功率 {point.success_rate}%</span>
          </div>
          ...
          <div className="mt-2 flex justify-between text-[11px] text-ops-overlay">
            ...
            <span>平均耗时 {Math.round(point.avg_duration_ms)} ms</span>
          </div>
