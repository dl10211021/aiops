...
export default function SessionSidebar() {
  ...
  return (
    <aside className="w-80 bg-ops-sidebar/86 border-r border-ops-surface0/80 flex flex-col shrink-0 overflow-hidden backdrop-blur-xl">
      <div className="border-b border-ops-surface0/80 p-4">
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <span className="block text-sm font-semibold text-ops-text">AI 会话工作台</span>
            <span className="text-[11px] text-ops-overlay">
              {model.sessionList.length} 个会话{model.runningCount > 0 ? ` / ${model.runningCount} 个执行中` : ''}
          ...
          <button
            ...
            className="shrink-0 rounded-lg bg-ops-accent px-3 py-1.5 text-xs font-semibold text-ops-dark transition-colors hover:bg-ops-accent/85"
          >
            + 新建
          </button>
        </div>
